...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    tools: {
      file_search: googleProvider.tools.fileSearch({
        fileSearchStoreNames: [orgDocs[0].fileSearchStoreName],
      }),
    },
    activeTools: ['file_search'],
    // v7 drops request bodies from step results by default; the LLM log needs it
    // to record the resolved generationConfig and systemInstruction.
  ...
  return createUIMessageStreamResponse({
    stream: toUIMessageStream({
      ...
      async onEnd({ messages: updatedMessages, finishReason }) {
        try {
          ...
          const { request } = await result.finalStep;
          const { generationConfig, systemPrompt: actualSystemPrompt } = extractRequestMetadata(
            request.body
          );
